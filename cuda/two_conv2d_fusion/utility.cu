          ...
          for (int kh_i = 0; kh_i < kh; kh_i++) {
            for (int kw_i = 0; kw_i < kw; kw_i++) {
              ...
              for (int ic_i = 0; ic_i < ic; ic_i++) {
                ...
                const T *in_ptr = (T*)input + nhwc(input_shape, input_index);
                const half *weight_ptr =
                    weight + nhwc(weight_shape, weight_index);
                sum += (float)(*in_ptr) * __half2float(*weight_ptr);
              }
            }
          }
          if (residual)
...
template void naive_conv_cpu(ConvAllParams params, half data);
template void naive_conv_cpu(ConvAllParams params, float data);
