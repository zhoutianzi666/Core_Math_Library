...
int main(void) {
  ...
  DATATYPE *input;
  int input_size = n * c * h;
  float epsilon = 0.000000567f;
  ...
  for (int i = 0; i < WARMUP; i++) {
    layernorm_gpu(n * c, h, dev_input, dev_input,dev_input, dev_out, epsilon);
  }
  ...
  naive_layernorm_cpu(out_cpu_fp32, input, n * c, h, epsilon);
